...
def analyze_audio(audio: np.ndarray, sample_rate: int, cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    threshold = float(cfg.get("score_threshold", 75))

    # Chain/engine ratio gate: reject engine-heavy sounds like motorcycles.
    min_ce = float(cfg.get("min_chain_engine_ratio", 0.80))
    ce_ratio = chain_ratio / max(engine_ratio, 0.001)
    has_chain_signature = ce_ratio >= min_ce

    detected = bool(enough_loudness and score >= threshold and has_chain_signature)
